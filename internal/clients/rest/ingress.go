...
func (c *Client) CreateIngressRequest(ctx context.Context) (git.AccessKey, error) {

	var marshalledPayload []byte

	url := c.BaseURL + "/test"

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, bytes.NewBuffer(marshalledPayload))
	if err != nil {
